...
int new_socket(char *serverAddressVar, int portno, int timeout_seconds) {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < loop; i++) {
        log_message(LOG_INFO, "Client loop %d", i);

        log_message(LOG_INFO, "Setting up socket");
        sockfd = new_socket(serverAddressVar, portno, timeout_seconds);
        log_message(LOG_INFO, "Socket setup complete");

        struct timeval start, end;
        log_message(LOG_INFO, "Starting timer");
        gettimeofday(&start, NULL);

        log_message(LOG_INFO, "Sending source file to server");
        send_file(sockfd, sourceCodeFileVar);
        log_message(LOG_INFO, "Source file sent to server");

        char server_response[1024];
        bzero(server_response, sizeof(server_response));
        log_message(LOG_INFO, "Reading server response");
        int server_response_len =
            read(sockfd, server_response, sizeof(server_response));
        log_message(LOG_INFO, "Server response read");
        ...
        log_message(LOG_INFO, "Timer stopped");

        log_message(LOG_INFO, "Server response: %s", server_response);
        ...
        close(sockfd);
        log_message(LOG_INFO, "Client socket closed");

        log_message(LOG_INFO, "Client loop %d complete", i);
    }
    gettimeofday(&loop_end, NULL);

    double loopTime =
        (double)(loop_end.tv_sec -
                 loop_start.tv_sec); // + (double)(loop_end.tv_usec -
                                     // loop_start.tv_usec) / 1000000.0;

    // Calculate average response time
    double averageResponseTime = totalResponseTime / (1.0 * successfulRes);
